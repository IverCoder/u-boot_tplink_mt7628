...
#endif
#if defined(TP_MODEL_WR841HPV5) || defined(EXTERNAL_RF_SWITCH)
/* only models with external PA and/or external LNA need to set it. */
		{
			RALINK_REG(RALINK_SYSCTL_BASE+0x60)|= ((0x3<<24));
		}
#endif
#if defined(TP_MODEL_WR841HPV5) || defined(EXTERNAL_PA_LNA)
/* only models with external PA and/or external LNA need to set it. */
		{
...
#if defined(DUAL_UBOOT_SUPPORT) && defined(FACTORY_UBOOT)
/* init gpio. */
#if defined(TP_MODEL_C55V1) || defined(TP_MODEL_C6V1) || defined(TP_MODEL_A1201V1)
	u32 gpiomode = le32_to_cpu(*(volatile u32 *)(RALINK_PIO_BASE + 0x4c));
	gpiomode |= 0x1 << (39 - 24);
	*(volatile u32 *)(RALINK_PIO_BASE + 0x4c) = gpiomode;

	/*for 7620, it is already on at boot time, just turn off it.*/
	setGpioData(7, 1);
#elif defined(TP_MODEL_C50V4)
	/* GPIO2 Mode */
	u32 gpiomode = le32_to_cpu(*(volatile u32 *)(RALINK_SYSCTL_BASE + 0x64));
	/*				   GPIO39 ~ GPIO44 */
	gpiomode &= ~0x0fff;
	/*				   GPIO39 ~ GPIO44 */
	gpiomode |= 0x0555;
	...
	*(volatile u32 *)(RALINK_SYSCTL_BASE + 0x64) = cpu_to_le32(gpiomode);
	...
	gpiomode = le32_to_cpu(*(volatile u32 *)(RALINK_PIO_BASE + 0x4));
	gpiomode |= 0x3f << (39 - 32);
	*(volatile u32 *)(RALINK_PIO_BASE + 0x4) = gpiomode;

	/*for 7628, it is already on at boot time, just turn off it.*/
	setGpioData(44, 1);
...
/*gpio has been changed after initiation, now renew it.*/
#if defined(TP_MODEL_C50V3)
	/* GPIO2 Mode */
	u32 gpiomode = le32_to_cpu(*(volatile u32 *)(RALINK_SYSCTL_BASE + 0x64));
	/*				   GPIO39 ~ GPIO44 */
	gpiomode &= ~0x0fff;
	...
	while (timer1 > 0) {
		...
		if (BootType != '4'/* goto cmd */ && BootType != '7' /* serial upgrade boot.bin */)
		{
...
#elif defined(TP_MODEL_C50V5)
				getGpioData(11, &data);
#elif defined(TP_MODEL_C55V1) || defined(TP_MODEL_A1201V1)
				getGpioData(13, &data);
#elif defined(TP_MODEL_C6V1)
				getGpioData(9, &data);
#else
                /* most MT7628N models set GPIO 38 as reset button conf, such as 8 series */
				getGpioData(38, &data);
#endif
			...
			if (icount == 0)
			{
				...
				/* light WPS led */
				setGpioData(42, 1);
#elif defined(TP_MODEL_C50V3) || defined(TP_MODEL_C50V4) || defined(TP_MODEL_C50V5)
				setGpioData(43, 0);
...
#if defined(TP_MODEL_WR840NV5)
				run_command("tftp 0x80060000 tp_recovery.bin;erase tplink 0x20000 0x3c0000;cp.b 0x80080000 0x20000 0x3c0000", 0);
#elif defined(TP_MODEL_WR840NV6)
				run_command("tftp 0x80060000 tp_recovery.bin;erase tplink 0x10000 0x3d0000;cp.b 0x80070000 0x10000 0x3d0000", 0);
#elif defined(TP_MODEL_WR840NV6_20)
				run_command("tftp 0x80060000 tp_recovery.bin;erase tplink 0x10000 0x3d0000;cp.b 0x80070000 0x10000 0x3d0000", 0);
#elif defined(TP_MODEL_WR845NV4)
				run_command("tftp 0x80060000 tp_recovery.bin;erase tplink 0x10000 0x3d0000;cp.b 0x80070000 0x10000 0x3d0000", 0);
#elif defined(TP_MODEL_WR841NV14)
				run_command("tftp 0x80060000 tp_recovery.bin;erase tplink 0x10000 0x3d0000;cp.b 0x80070000 0x10000 0x3d0000", 0);
#elif defined(DUAL_UBOOT_SUPPORT) && defined(FACTORY_UBOOT)
				memset(runcmd, 0, 128);
				sprintf(runcmd, "tftp 0x80060000 tp_recovery.bin;erase tplink %x %x;cp.b %x %x %x",
					FACTORY_UBOOT_SIZE, 0x7c0000 - FACTORY_UBOOT_SIZE,
					0x80060000 + FACTORY_UBOOT_SIZE, FACTORY_UBOOT_SIZE, 0x7c0000 - FACTORY_UBOOT_SIZE);
				run_command(runcmd, 0);
				ubootArgs * pargs = 0x80060000;
				memset(pargs, 0, sizeof(ubootArgs));
				raspi_erase_write(pargs, DUAL_UBOOT_ARGS_ADDR, sizeof(ubootArgs));
#else
...
#if defined(DUAL_UBOOT_SUPPORT) && defined(FACTORY_UBOOT)
					void	(*uboot_sec)();
					uboot_sec = CFG_SECLOADER_ADDR;
					flush_cache(CFG_SDRAM_BASE, gd->ram_size);
					uboot_sec();
#else /* defined(DUAL_UBOOT_SUPPORT) && defined(FACTORY_UBOOT) */
...
#endif /* defined(DUAL_UBOOT_SUPPORT) && defined(FACTORY_UBOOT) */
	}
	else {
		char *argv[4];
		int argc= 3;
